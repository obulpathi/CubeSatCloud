...
class Transceiver(object):
    ...
    def receive(self, llpacket):
        # model packet corruption aka BAD PACKET
        print "Received: " + str(llpacket)
        choice = randint(1, 10000)
        if choice < 10:
            llpacket.flags = LL_BAD_PACKET
        # if we received a bad packet
        if llpacket.flags == LL_BAD_PACKET:
            return
...
if __name__ == "__main__":
    ...
    packet = Packet(Cube("Sender"), Cube("Receiver"), Cube("Source"), Cube("Destination"), "DATA", "payload", 100, 100*MB)
    x.transmit(packet)
    time = 0
    while not finished:
